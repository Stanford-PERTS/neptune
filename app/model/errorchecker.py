...
The waters I govern are restless. More information is available on the
dashboard.

    https://console.cloud.google.com/logs/viewer?project=neptuneplatform&minLogLevel=500
...
"""
    ...
    def redact_error_text(self, resource):
        if not isinstance(resource, basestring):
            return resource

        # Handle participant name queries from the portal.
        if '/api/participants' in resource and 'name=' in resource:
            pattern = r'([?&])name=([^&\n]*)'
            replace = r'\1name=<REDACTED>'
            resource = re.sub(pattern, replace, resource)

        # Handle roster queries sent to Copilot.
        if '/api/codes/' in resource and '/participants/' in resource:
            pattern = r'/participants/[^?#/]+'
            replace = r'/participants/<REDACTED>'
            resource = re.sub(pattern, replace, resource)
    ...
    def mail_log(self):
        ...
        for to in self.to_addresses:
            email = Email.create(
                to_address=to,
                subject=self.redact_error_text(subject),
                body=self.redact_error_text(body),
            )
            email.put()
