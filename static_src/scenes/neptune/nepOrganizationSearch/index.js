...
const nepOrganizationSearch = ngModule => {
  ngModule.directive('nepOrganizationSearch', (User, Organization) => {
    'use strict';
    ...
    function link(scope, element, attrs) {
      ...
      scope.selectPlaceAsOrg = function (place) {
        ...
        // and their place id.
        Organization.getAllPlaceIds(orgs => {
          scope.orgsByPlace = keyBy(orgs, 'google_maps_place_id');

          // Convert from a google maps place to one of our orgs.
          const orgParams = Organization.paramsFromPlace(place);
          ...
          if (place.place_id in scope.orgsByPlace) {
            ...
            scope.selectedOrganization.uid =
              scope.orgsByPlace[place.place_id].uid;
            scope.orgChange({
              org: scope.selectedOrganization,
              orgIsValid: false,
              invalidMessage:
                'This organization has already been ' +
...
export default nepOrganizationSearch;
