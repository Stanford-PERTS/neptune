...
window.PERTS = (function(PERTS) {
  ...
  function getParticipationInfo(program, team, cycle) {
    var info = {};
    if (program.use_classrooms) {
      info.ppnPct = getParticipationPct(team, recent);
      info.ppnStr = info.ppnPct === null ? 'empty roster' : info.ppnPct + '%';
      info.displayStatus =
        info.ppnPct >= MAX_BAD_PPN
          ? info.ppnPct >= MIN_GOOD_PPN
            ? 'good'
            : ''
          : 'bad';
    } else {
      info.ppnStr = String(cycle.students_completed || 0);
      info.displayStatus = '';
    }
  ...
  function getDisplayCycles(program, team, cycles) {
    var recent;
    ...
    if (recent) {
      ...
      // Participation.
      var ppnInfo = getParticipationInfo(program, team, recent);
      recent.ppnPct = ppnInfo.ppnPct;
      recent.ppnStr = ppnInfo.ppnStr;
      recent.displayStatus = ppnInfo.displayStatus;
      ...
      if (recent.ordinal > 1) {
        ...
        if (previous) {
          // Participation.
          var ppnInfo = getParticipationInfo(program, team, previous);
          previous.ppnPct = ppnInfo.ppnPct;
          previous.ppnStr = ppnInfo.ppnStr;
          previous.displayStatus = ppnInfo.displayStatus;
        }
      }
    }
  ...
  function wrangleForTable(loaded) {
    ...
    $.each(teams, function(i, team) {
      ...
      var displayCycles = getDisplayCycles(
        loaded.reportData.program,
        team,
        teamCycles,
      );
  ...
  function populateTableCycleless(loaded, tritonDomain) {
    var table = $('#participation-summary-table');
  ...
  PERTS.buildTable = function buildTable(location, orgId) {
    ...
    return loadData(token, nep, tri, orgId)
      ...
      .then(function(loaded) {
        if (loaded.reportData.program.use_cycles) {
          return populateTable(loaded, tri);
        }
        return populateTableCycleless(loaded, tri);
      })
      .then(insertTerms)
      .then(function() {
